...
void Triangulation2D_Delaunay_Bowyer_Watson::computeTriangulation() {
	...
	Triangle supertriangle(Point(-10 * WIDTH, -10 * HEIGHT), Point(10 * WIDTH, -10 * HEIGHT), Point(WIDTH / 2, 10 * HEIGHT));
	...
	for(auto& point : _points) {
		...
		for(auto& edge : edgeBuffer) {
			_triangles.push_back(Triangle(point, edge.getStartPoint(), edge.getEndPoint()));
		}
	}
	...
	_triangles.erase(std::remove_if(_triangles.begin(), _triangles.end(), [supertriangle](const Triangle& triangle) {
